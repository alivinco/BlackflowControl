...
def app_context(inst_name):
    ...
    msg = IotMsg.new_iot_msg("blackflow", "command", "blackflow", "context_get")
    response = sync_async_client.send_sync_msg(msg.get_dict(), "/app/blackflow/%s/commands" % inst_name, "/app/blackflow/%s/events" % inst_name, timeout=10,
                                               correlation_msg_type="blackflow.context", correlation_type="MSG_TYPE")
    return render_template('blackflow/context.html', bf_response=response, bf_inst_name=inst_name, global_context=global_context,
...
def apps(inst_name):
    ...
    msg = IotMsg.new_iot_msg("blackflow", "command", "blackflow", "get_apps")
    response = sync_async_client.send_sync_msg(msg.get_dict(), "/app/blackflow/%s/commands" % inst_name, "/app/blackflow/%s/events" % inst_name, timeout=10,
...
def app_instances(bf_inst_name):
    log.info("Blackflow Apps")
    msg = IotMsg.new_iot_msg("blackflow", "command", "blackflow", "get_app_instances")
    response = sync_async_client.send_sync_msg(msg.get_dict(), "/app/blackflow/%s/commands" % bf_inst_name, "/app/blackflow/%s/events" % bf_inst_name, timeout=10,
    ...
    return render_template('blackflow/app_instances.html', bf_response=response, bf_inst_name=bf_inst_name, global_context=global_context,
                           format_time=utils.format_iso_time_from_sec)
...
def app_instance_config(inst_name):
    ...
    return render_template('blackflow/app_instance_config.html', bf_inst_name=inst_name, inst_id=inst_id, app_name=app_name, global_context=global_context,
                           format_time=utils.format_iso_time_from_sec)
...
def blackflow_proxy(inst_name):
    ...
    if request_type == "one_way":
        ...
        response = sync_async_client.send_sync_msg(request_payload, request_topic, response_topic, sync_request_timeout, correlation_msg_type=correlation_msg_type,
...
def containers_api(inst_name):
    response = sync_async_client.send_sync_msg(msg, "/app/blackflow/%s/commands" % inst_name, "/app/blackflow/%s/events" % inst_name, timeout=5,
                                               correlation_msg_type="blackflow.apps", correlation_type="MSG_TYPE")
...
def app_instance_config_api(inst_name):
    ...
    response = sync_async_client.send_sync_msg(msg.get_dict(), "/app/blackflow/%s/commands" % inst_name, "/app/blackflow/%s/events" % inst_name, timeout=5,
                                               correlation_msg_type="blackflow.apps", correlation_type="MSG_TYPE")
    ...
    msg = IotMsg.new_iot_msg("blackflow", "command", "blackflow", "get_app_instances")
    all_app_instances = sync_async_client.send_sync_msg(msg.get_dict(), "/app/blackflow/%s/commands" % inst_name, "/app/blackflow/%s/events" % inst_name, timeout=5,
                                                        correlation_msg_type="blackflow.app_instances", correlation_type="MSG_TYPE")
    if inst_id:
...
def blackflow_analytics(inst_name):
    ...
    msg = IotMsg.new_iot_msg("blackflow", "command", "blackflow", "analytics_get")
    response = sync_async_client.send_sync_msg(msg.get_dict(), "/app/blackflow/%s/commands" % inst_name, "/app/blackflow/%s/events" % inst_name, timeout=5,
