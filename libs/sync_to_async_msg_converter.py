...
class SyncToAsyncMsgConverter:
    ...
    def send_sync_msg(self,msg,request_topic,response_topic,timeout=30):

        """
        The method sends message over async messaging subsystem and is waiting for response (blocking) .
        The method is executed in callers thread , which means it block the thread .
        :param msg:
        :param request_topic:
    ...
    def on_message(self,topic,msg):
        ...
        :param msg:
        """
        # small optimisation. Skip messages if there are no request waiting for response.
