...
class MessageManager:
    def __init__(self):
        ...
        self.msg_to_log_file_path = os.path.join(self.app_root_path, "configs", "msg_to_log.json")

        self.msg_class_mapping = None
        self.address_mapping = None
        self.msg_to_log_mapping = None
        self.services_to_msg_mapping = None

        self.reload_all_mappings()

        self.global_configs = self.load_mapping("global")
    ...
    def load_mapping(self, mapping_name):
        file_path = os.path.join(self.app_root_path, "configs", "%s.json" % mapping_name)
        log.debug("Loading mapping from file %s" % file_path)
        jobj = json.load(file(file_path))
        return jobj

    def serialize_mapping(self, mapping_name):
        file_path = os.path.join(self.app_root_path, "configs", "%s.json" % mapping_name)
        log.info("Serializing mapping to file %s" % file_path)
        if mapping_name == "global":
            sjobj = self.global_configs
        else :
            sjobj = getattr(self,mapping_name)

        with open(file_path, "w") as f:
            f.write(json.dumps(sjobj, indent=True))

    def reload_all_mappings(self):
        self.msg_class_mapping = self.load_mapping("msg_class_mapping")
        self.address_mapping = self.load_mapping("address_mapping")
        self.msg_to_log_mapping = self.load_mapping("msg_to_log_mapping")
        self.services_to_msg_mapping = self.load_mapping("services_to_msg_mapping")
    ...
    def update_address_mapping(self, id, name, msg_class, msg_type, address, override_properties="", override_value_path="", record_history=False, serialize=True,
        ...
        if id:
            ...
            self.address_mapping.append({"id": new_id, "msg_class": msg_class, "address": address, "name": name, "msg_type": msg_type, "record_history": record_history,
                                         "override_properties": override_properties, "override_value_path": override_value_path, "key": key, "transport": transport})

        if serialize: self.serialize_mapping("address_mapping")
    ...
    def add_address_to_mapping(self, address, msg_class, serialize=True):
        ...
        if not self.check_if_address_exists(address, msg_class):
            new_id = self.get_new_addr_id()
            if "event" in address:
                self.address_mapping.append(
                    {"id": new_id, "msg_class": msg_class, "address": address, "name": msg_class, "msg_type": "event", "record_history": False, "group_name": "",
                     "override_properties": "", "override_value_path": "", "key": self.generate_key(msg_class, address)})
            elif "command" in address:
                self.address_mapping.append(
                    {"id": new_id, "msg_class": msg_class, "address": address, "name": msg_class, "msg_type": "command", "group_name": "", "record_history": False,
                     "override_properties": "", "override_value_path": "", "key": self.generate_key(msg_class, address)})
    ...
    def remove_address_from_mapping(self, id):
        ...
        self.serialize_mapping("address_mapping")
        ...
        for service in list_of_services:
            if service["Type"] in self.services_to_msg_mapping:
                ...
                for event_msg_class in service_map["events"]:
                    ...
                    if device_name:
                        name = device_name + "=>" + event_msg_class
                    else:
                        name = event_msg_class
                ...
                for command_msg_class in service_map["commands"]:
                    ...
                    if device_name:
                        name = device_name + "=>" + command_msg_class
                    else:
                        name = event_msg_class
