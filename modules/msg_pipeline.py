...
class MsgPipeline():
    ...
    def process_event(self, address, payload):
        ...
        if msg_class:
            ...
            log.error(
                "The system can't identify message class . Therefore message processing stoped and the message will be skipped.")
        ...
        # elif (not addr_is_registered and msg_class_is_registered) or (addr_is_registered and msg_class and not msg_class_is_registered) :
            ...
            self.cache.put_msg_class_for_approval(address, payload, msg_class,
                                                  "Message class is unknown and has to be approved")
            return {"success": True, "code": 1,
                    "text": "Unknown message class:" + msg_class + ". It will be added for user approval"}
            ...
            log.info(
                "Address :" + address + " is unknown and command class can't be extracted from the message.The message will be skipped")
            return {"success": False, "code": 1,
                    "text": "Address :" + address + " is unknown and command class can't be extracted from the message.The message will be skipped"}
        else:
    ...
    def __get_msg_class_from_msg(self, payload):
        ...
        :return:
        """
        if self.msg_man.global_configs["use_default_class_lookup"]:
            if "command" in payload:
                msg_class = payload["command"]["@type"] + "." + payload["command"]["subtype"]
            else:
                msg_class = payload["event"]["@type"] + "." + payload["event"]["subtype"]
            return msg_class
        ...
        try:
            ...
            if "override_value_path" in address_map:
                ...
                if override_path:
                    ...
                    ex_value = self.msg_man.get_value_from_msg(payload, override_path)
            ...
            if not override_path:
                for key, value in ui_mapping.items():
                    if ("path" in key) and value:
                        ...
                        ex_value = self.msg_man.get_value_from_msg(payload, value)
                        log.debug("Extracted data is = " + str(ex_value))
